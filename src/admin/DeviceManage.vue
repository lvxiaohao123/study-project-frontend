      ...
      <el-table :data="devices" style="width: 100%">
        ...
        <el-table-column label="操作">
          <template #default="{ row }">
            ...
            <el-button type="primary" @click="goToDeviceUpdate(row.id)">编辑</el-button>
            <el-button type="primary" @click="DeleteDevice(row.id)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
...
const goToDeviceUpdate = (deviceId) => {
  router.push(`/UpdateDevice/${deviceId}`);
};

const DeleteDevice = async (deviceId) => {
  try {
    await axios.post(`/api/device/deleteDeviceByID?id=${deviceId}`);
